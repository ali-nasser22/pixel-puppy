...
export default function FavPuppy({image,name,onRemove}:{image:string,name:string,onRemove:()=>void}) {
    return (
        <li className="relative flex items-center overflow-clip rounded-md bg-white shadow-sm ring ring-black/5 transition duration-100 starting:scale-0 starting:opacity-0">
            <Image
                ...
                src={image}
            />
            <p className="px-3 text-sm text-slate-800">{name}</p>
            <button className="group h-full border-l border-slate-100 px-2 hover:bg-slate-100" onClick={onRemove}>
                <svg
